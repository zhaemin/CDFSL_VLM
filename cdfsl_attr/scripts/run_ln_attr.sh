...
do
    ...
    do
        for SEED in 1
        do
        CUDA_VISIBLE_DEVICES=3 python main.py --shots ${SHOOT} --dataset ${DATA} --seed ${SEED} --mode 'ln_only_attr' --exp_name "ln_attr${NUM_ATTR}" --root_path ${ROOT_PATH} --num_attr ${NUM_ATTR}
        done
    done
done
