...
class SlotMultiheadAttention(nn.Module):
    def __init__(self, existing_mha):
        super().__init__()
...
class AttentionPooling(nn.Module):
    def __init__(
        ...
        self.attr_probe = nn.Parameter(torch.randn(1, 8, self.embed_dim))
    ...
    def init_slot_attn(self):
        slotattn = SlotMultiheadAttention(self.attn)
        self.attn = slotattn
...
class ResidualAttentionBlock(nn.Module):
    ...
    def _call_attn(
        ...
        elif isinstance(self.attn, SelfAttentionLoRA):
            return self.attn(q_x, attn_mask=attn_mask)
        else:
...
class TextTransformer(nn.Module):
    def __init__(
        ...
        #self.family_prompt = nn.Parameter(torch.randn(70, self.prompt_length, width))
    ...
    def init_prompts(self, tokenized_template, random_init=False):
        if random_init:
            self.n_token_prompt = 4
            ctx_vectors = torch.empty(1, self.n_token_prompt, self.width, dtype=float)
            nn.init.normal_(ctx_vectors, std=0.02)
            self.global_prompt = nn.Parameter(ctx_vectors)
            print('n_token_prompt: ', self.n_token_prompt)
        else:
            self.template_init_tokens = int(tokenized_template.argmax(dim=-1)) - 1
            self.n_token_prompt = self.template_init_tokens
            print('n_token_prompt: ', self.n_token_prompt)
            self.global_prompt = nn.Parameter(torch.randn(1, self.n_token_prompt, self.width))
            
            embedding = self.token_embedding(tokenized_template)
            global_prompt_init = embedding[:, 1 : 1 + self.template_init_tokens, :]

            self.global_prompt.data[:, : self.template_init_tokens].copy_(global_prompt_init.clone())

    def forward(self, text, prompt=False):
        B = text.shape[0]
        ...
        if prompt:
            global_prompt = self.global_prompt.repeat(B, 1, 1)
            suffix = x[:, :1, :] # SOS
            prefix = x[:, 1:-self.n_token_prompt, :] # prompt length만큼 뒤를 자름
            x = torch.cat([suffix, global_prompt, prefix], dim=1)
        
        attn_mask = self.attn_mask
        if attn_mask is not None:
            attn_mask = attn_mask[:seq_len, :seq_len]
        
        x = x + self.positional_embedding[:seq_len] # 100 32 1024
        x = self.transformer(x, attn_mask=attn_mask)

        x = self.ln_final(x)

        if prompt:
            eot_pos = text.argmax(dim=-1) + self.n_token_prompt
            pooled = x[torch.arange(x.shape[0]), eot_pos]
        else:
...
class CLIP(TextTransformer):
    ...
    def encode_text(self, text, normalize: bool = False, prompt = False):
        x = super().forward(text, prompt)
        return F.normalize(x, dim=-1) if normalize else x
