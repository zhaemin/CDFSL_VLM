...
def visualize_query_similarity(args, attns, save_dir='./vis'): 
    num_queries = attns.shape[0] # (num_queries, N)

    # 각 query를 1D 벡터로 flatten
    attn_flat = attns.detach().cpu()
    ...
    plt.savefig(f"{save_dir}/query_sim_{args.exp_name}.png")
    plt.close()
...
def run_ln_only_attr_slot(args, clip_model, logit_scale, dataset, train_loader, val_loader, test_loader):
    ...
    for param in clip_model.visual.attn_pool.attn.q_proj.parameters():
        new_trainable_params.append(param)
    ...
    queries = torch.cat([clip_model.visual.attn_pool.probe, clip_model.visual.attn_pool.attr_probe], dim=1).squeeze(0)
    visualize_query_similarity(args, queries)

    print('after_trained: ', clip_model.visual.attn_pool.attr_probe)
    '''
    # qkv projection visualization
