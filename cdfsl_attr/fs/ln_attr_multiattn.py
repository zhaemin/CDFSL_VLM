...
from fs.utils.eval_utils import tokenize_texts, cls_acc, evaluate_attr, evaluate_attr_multiattn
...
def train_epoch(clip_model, optimizer, scheduler, scaler, train_loader, test_loader, dataset, tokenized_texts, count_iters, total_iters, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        attr_cosine_similarity = clip_model.logit_scale.exp() * torch.einsum('bd, bcd -> bc', image_features_global, attr_prototype)
        
        loss = F.cross_entropy(attr_cosine_similarity, target)
        ...
        acc_train += cls_acc(attr_cosine_similarity, target) * target.shape[0]
...
def run_ln_only_attr(args, clip_model, logit_scale, dataset, train_loader, val_loader, test_loader):
    ...
    trainable_params.append(clip_model.visual.attn_pool.attr_probe)
    ...
    '''
    # in_proj_weight 분리
    W = clip_model.visual.attr_pool.attn.in_proj_weight.detach().cpu()
    dim = clip_model.visual.width
    W_Q, W_K, W_V = W[:dim], W[dim:2*dim], W[2*dim:]

    # in_proj_bias 분리
    b = clip_model.visual.attr_pool.attn.in_proj_bias.detach().cpu()
    ...
    if args.setting == "base2new":
        ...
        acc_test_base, attr_acc_test_base = evaluate_attr_multiattn(
        ...
        acc_test_novel, attr_acc_test_novel  = evaluate_attr_multiattn(
        ...
        acc_test, gloabl_acc_test, attr_acc_test = evaluate_attr_multiattn(
