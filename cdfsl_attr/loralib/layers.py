...
def scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0,
    ...
    return attn_weight @ value, attn_weight.mean(dim=1)
...
class SelfAttentionLoRA(nn.Module):
    ...
    def __init__(
        ...
        super(SelfAttentionLoRA, self).__init__()
        
        self.rope = existing_mha.rope
...
class PlainMultiheadAttentionLoRA(nn.Module):
    ...
    def forward_module(
        ...
        attn_output, attn_weight = self.scaled_dot_product_attention(q, k, v, attn_mask, dropout_p, is_causal)
        attn_output = attn_output.permute(2, 0, 1, 3).contiguous().view(bsz * tgt_len, embed_dim)
        attn_output = self.proj(attn_output)
        attn_output = attn_output.view(tgt_len, bsz, attn_output.size(1))
        if self.batch_first and is_batched:
            if need_weights:
                return attn_output.transpose(1, 0), attn_weight
            else:
...
class MultiheadAttentionLoRAWOQproj(PlainMultiheadAttentionLoRA):
    def forward_module(
            self,
            query,
            key,
